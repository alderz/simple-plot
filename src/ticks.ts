...
export const calculateTickSize = (min: number, max: number, noTicks: number): number => {
    ...
    // Magnitude is the tenth power that most closely resembles the range we are trying to express.
...
export const getDefaultYTicks = (min: number, max: number, n: number): Tick[] => {
    ...
    return ticks.map(
        (t: number): Tick => ({position: t, label: t.toFixed(decimalPlaces < 0 ? -decimalPlaces : 0)})
    );
};
